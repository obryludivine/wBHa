...
vignette: >
  %\VignetteIndexEntry{GSE90102-tutorial}
...
## Maf of all SNPs
```{r MafofallSNPs, fig.align='center', fig.width=7, fig.height=5}
...
```{r Histogram_rejects, fig.align='center', fig.width=7, fig.height=5}
...
### UpsetR graph
```{r UpsetR, fig.align='center', fig.width=7, fig.height=5}
UpsetR_graph <- upset(fromList(rej_list), order.by="freq", mainbar.y.label="SNP",
                      sets.x.label="Nb of rejections", nsets=8)
UpsetR_graph
```

We note that the BH, qvalue and swfdr procedures reject the same variants $\Rightarrow$ reduction of the number of procedures. Moreover, we note that the FDRreg procedure rejects a small number of markers as was observed with the simulation data in the case of quantitative phenotypes.
...
```{r Histogram_subgoups, fig.align='center', fig.width=7, fig.height=5}
...
subgroup_hist
```
We can note that wBHa focuses on rare markers (less than 2%). However, although this is also the case for the other weighted procedures combining the two types of approaches, these are less powerful for rare variants than wBHa.
...
## Venn diagram
```{r Venn, fig.align='center', fig.width=7, fig.height=5}
